...
object FlightRecord {
  ...
  /**
   *
   * The most basic data cleaning (Not a single row of data has been removed)
   *
   * 1. drop (
   * "ArrTime",
   * "ActualElapsedTime",
   ...
   */
  def apply(row: Row): FlightRecord = {
    ...
    def getStringValue(index: Int): Option[String] = {
    ...
    var depTime = getIntValue(depTimeIndex).getOrElse(0)
    depTime = (60 * ((depTime - (depTime % 100)) / 100)) + (depTime % 100)
    var crsDepTime = getIntValue(crsDepTimeIndex).getOrElse(0)
    crsDepTime = (60 * ((crsDepTime - (crsDepTime % 100)) / 100)) + (crsDepTime % 100)
    var crsArrTime = getIntValue(crsArrTimeIndex).getOrElse(0)
    crsArrTime = (60 * ((crsArrTime - (crsArrTime % 100)) / 100)) + (crsArrTime % 100)
    ...
    FlightRecord(year, month, dayOfMonth, dayOfWeek, depTime, crsDepTime, crsArrTime, uniqueCarrier, flightNum, tailNum, crsElapsedTime, depDelay, origin, dest, distance, taxiOut, cancelled, cancellationCode, arrDelay)
  }
